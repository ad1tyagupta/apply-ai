...
/**
 * doctor.mjs — Setup validation for ApplyAI
...
function checkUserPreferences() {
  ...
  return {
    pass: false,
    label: 'config/user-preferences.yml not found',
    fix: [
      'Run: cp config/user-preferences.example.yml config/user-preferences.yml',
      'Then set geography, search mode, and company preferences',
    ],
  };
}
...
function checkIntakeFolders() {
  const rawDir = join(projectRoot, 'intake', 'raw');
  const normalizedDir = join(projectRoot, 'intake', 'normalized');
  if (existsSync(rawDir) && existsSync(normalizedDir)) {
    return { pass: true, label: 'intake/raw and intake/normalized ready' };
  }
  return {
    pass: false,
    label: 'intake folders missing',
    fix: 'Create intake/raw and intake/normalized or re-run setup',
  };
}

function checkFonts() {
  const fontsDir = join(CORE_DIR, 'fonts');
...
async function main() {
  ...
  if (failures > 0) {
    ...
    console.log('Result: All checks passed. You\'re ready to go! Open Codex in this workspace to start onboarding.');
    console.log('');
    process.exit(0);
  }
}
